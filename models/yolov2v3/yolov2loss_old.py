...
class YOLOv2Loss(nn.Module):
    ...
    def forward(self, p, targets):
        ...
        for outputs in p:
            ...
            # print(f"outputs.shape: {outputs.shape} - outputs[..., :4].shape: {outputs[..., :4].shape}")
            ...
            # --------------------------------------
            # box loss
            # print(f"pred_deltas.shape: {pred_deltas.shape} - pred_deltas[bi].shape: {pred_deltas[bi].shape} - box_mask[bi].shape: {box_mask[bi].shape}")
            pred_deltas_ = pred_deltas[box_mask > 0]
            box_target_ = box_target[box_mask > 0]
            box_scale_ = box_scale[box_mask > 0]
            box_loss = F.mse_loss(pred_deltas_, box_target_, reduction='none')
            box_loss *= box_scale_
            ...
            pred_probs_ = pred_probs[class_mask > 0]
            class_target_ = class_target[class_mask > 0]
            class_loss = F.cross_entropy(pred_probs_, class_target_, reduction='sum')
            ...
            # print(box_loss, obj_iou_loss, noobj_iou_loss, class_loss)
